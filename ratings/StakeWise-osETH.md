...
### SECURITY (40% Weight) -- Score: 360/360 (100%)
...
| S-SC-10 | Are real-time security monitoring and alerting systems in place? | Internal monitoring and alerting systems are in place for all core components: DAO governance, oracle votes, osETH APY changes, deposits/withdrawals, with some exposed publicly. Critical smart contracts are immutable. | 9 | 9 | Non-Improvable (optimal)| [P0] Publicly exposed monitoring systems for (oracles)[https://status.stakewise.io/] and [DAO governance](https://discord.com/channels/613394789882134529/851471420781494342) |
| S-SC-11 | Are automatic safety controls (e.g. pause, circuit breakers) triggered by monitoring alerts? | KeeperRewards.sol contract automatically caps osETH APY to protect holders from inflation attacks (circuit breaker). | 9 | 9 | Non-Improvable (optimal) | [P1] KeeperRewards.sol [automatically caps osETH APY](https://github.com/stakewise/v3-core/blob/main/contracts/keeper/KeeperRewards.sol) to minimize effect of theoretical oracle quorum compromise |

**Smart Contract Security Subtotal:**
- Raw scores: 9+9+9+9+9+9+9+9+9+9+9 = 99/99
- Adjusted: (99/99) x 180 = **180/180 (100%)**
...
| S-KM-08 | Is there a tested incident playbook for admin-key compromise or signer loss? | No incidents of key compromise or signer losses have been reported to date. There is no admin key controlling the protocol. Remaining upgradable contracts are subject to changes only via DAO governance or emergency multisig. Replacement of multisig signers and oracles has precedent according to the industry best practices (flag change ahead of time + execute change + keep records). | 9 | 9 | Non-Improvable (optimal)| [P1] Examples of industry playbook for critical rotations being followed for [multisig](https://forum.stakewise.io/t/swip-13-rotate-stakewise-dao-multisig-signers/809) and [oracle set](https://forum.stakewise.io/t/swip-20-change-the-oracle-config-on-gnosis-chain-to-replace-an-oracle/1624) |

**Key Management Subtotal (7 of 8 scored):**
- Raw scores: 9+9+9+9+9+9+9 = 63/63
- Adjusted: (63/63) x 180 = **180/180 (100%)**

**Security Total: 180 + 180 = 360/360 (100%)**

### STRATEGY (30% Weight) -- Score: 264/270 (97.8%)

#### Protocol Mechanics (5% weight, 45 max points)

| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| ST-PM-01 | What are the sources of yield and is it aligned with the mandate? | 100% of yield from Ethereum consensus + execution layer staking rewards (attestation, block proposals, MEV). 5% fee to DAO treasury. Fully aligned with ETH liquid staking mandate. ~2.5-3% APY. | 9 | 9 | Non-Improvable (optimal) | [P0] OsTokenVaultController: avgRewardPerSecond, [P1] [osToken docs](https://docs.stakewise.io/docs/ostoken/how-ostoken-works) |
...
| ST-IC-02 | How redundant and battle-tested are the oracle and bridge setups? | Internal oracle: 11 operators, 7-of-11 consensus, open-source code. External: RedStone aggregates 3 DEX sources. Chainlink on L2s. No bridge. | 9 | 9 | Non-Improvable (optimal) | [P1] [Oracle GitHub](https://github.com/stakewise/oracle), [P1] RedStone |
| ST-IC-03 | Are off-chain infrastructure providers certified by standard IT security audits? | Seven out of eleven companies serving as oracles report having ISO 27000/27001 or SOC2 Type II certifications (Deutsche Telekom, Finoa, DSRV, P2P.org, Chorus One, Sensei Node, StakeFish). | 9 | 9 | Non-Improvable (optimal) | [P1] [Deutsche Telekom MMS](https://www.telekom-mms.com/ueber-uns/unternehmen), Finoa, [DSRV](https://kr.aving.net/news/articleView.html?idxno=1770861), [P2P.org](https://p2p.org/economy/p2p-org-achieves-soc-2-type-ii-certification/), [Chorus One](https://security.chorus.one/), [Sensei Node](https://www.senseinode.com/en/security), [StakeFish](https://blog.stake.fish/stakefish-completes-soc-2-type-ii-certification-for-non-custodial-blockchain-staking-services/) |
...
| ST-IC-07 | Are validators diverse geographically and by operator? | osETH is minted from Vaults run by a diverse node operator set: StakeWise Labs (Genesis Vault), Chorus One (Chorus One Max Vault), NodeSet - a constellation of 100+ geographically distributed home operators (NodeSet Private Vault), etc. osETH minting and Vault creation (new operators joining the protocol) are permissionless. | 9 | 9 | Non-Improvable (optimal) | [P0] [StakeWise Vaults page](https://app.stakewise.io/vaults) |
| ST-IC-08 | Can any single infra failure block withdrawals or cause losses? | No. Withdrawals are permissionless via FIFO queue on every Vault contract. Oracle failure delays finalization but does not block claiming of already-finalized requests. Multiple redundancy layers exist. No single component is a permanent single point of failure for exits. | 9 | 9 | Non-Improvable (optimal) | [P1] [Withdrawal Queue](https://github.com/stakewise/v3-core/blob/main/contracts/libraries/ExitQueue.sol) |

**Infra Counterparty Subtotal:**
- Raw scores: 9+9+9+9+9+9+9+9 = 72/72
- Adjusted: (72/72) x 45 = **45.0/45 (100%)**
...
| ST-L-02 | Is there enough liquidity to redeem close to 100% of TVL within stated withdrawal time? | osETH total supply (TVL) ~$450M. DEX 24h volume moderate (~$0.5M, March 2026). In-protocol redemption at fair rate still works. Full TVL exit requires validator exits (8-day + Ethereum queue). | 9 | 9 | Non-Improvable (optimal) | [P3] [CoinGecko: ~$0.5m 24h volume](https://www.coingecko.com/en/coins/stakewise-staked-eth) (March 2026), [P0] In-protocol redemption |
...
| ST-L-05 | How does exit liquidity behave during volatility and network congestion? | Balancer hack (Nov 2025) destroyed primary DEX liquidity pool, later rebuilt in January 2026 on Fluid. DEX 24h volume steady around $0.5M. osETH holders unaffected. Protocol behaved as expected through every major crypto-wide stress event since launch in 2021, with no outages or liquidity shortages reported. | 9 | 9 | Non-Improvable (optimal) | [P3] [CoinGecko: ~$0.5M 24h volume (March 2026)](https://www.coingecko.com/en/coins/stakewise-staked-eth), [P1] Balancer hack Nov 2025, [P1] [New liquidity established on Fluid in Jan 2026](https://x.com/stakewise_io/status/2012138263344300047?s=20) |
...
| ST-L-08 | What is the liquidity depth of collateral and receipt tokens? | osETH market cap ~$407-450M. 24h DEX volume ~$0.5M (March 2026) is on par with 2024 levels (~$636K). Post-Balancer hack (Nov 2025), primary liquidity pool is on Fluid. In-protocol redemption at fair rate remains the viable exit. | 9 | 9 | Non-Improvable (optimal) | [P3] [CoinGecko: ~$0.5M 24h volume (March 2026)](https://www.coingecko.com/en/coins/stakewise-staked-eth), [P0] In-protocol redemption |
| ST-L-09 | What is the risk of bank-run scenarios under full utilization? | 10% overcollateralization buffer provides protection. Mass exit triggers validator exits (8-day + Ethereum queue). No leverage in base product. Significant osETH concentration in Aave coming from a smart contract-managed leveraged staking strategy. Mass exit relies entirely on in-protocol redemption + validator exits. Tested at scale between July 20th and August 11th 2025, processing 33% of TVL (91K osETH) in redemptions without issues, per Etherscan. | 9 | 9 | Non-Improvable (optimal) | [P1] [osETH supply check](https://etherscan.io/tokencheck-tool?t=0xf1c9acdc66974dfb6decb12aa385b9cd01190e38) |
...
**Strategy Total: 45.0 + 45.0 + 45 + 45.0 + 45.0 + 39.0 = 264/270 (97.8%)**

### OPERATIONS (30% Weight) -- Score: 256.5/270 (95.0%)

#### Governance (7.5% weight, 67.5 max points)

| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| O-G-01 | What governance model controls protocol changes? | Hybrid: SWISE token voting via Snapshot + SafeSnap execution through 4-of-7 Gnosis Safe. SafeSnap provides 48h delay. 200K SWISE bond for proposals, 3M SWISE quorum. DAO controls only a minimal set of smart contracts - only VaultFactory.sol and new Vault versions. Vault upgrades to new versions require dual approval - update made available via DAO governance, with opt-in required from Vault owners to upgrade, effectively vetoing any malicious upgrade. Multisig can execute within governance-approved bounds but also reject. | 9 | 9 | Non-Improvable (optimal) | [P2] [Snapshot](https://snapshot.box/#/s:stakewise.eth), [P1] SafeSnap parameters |
| O-G-02 | How concentrated is voting power among top holders? | 1B SWISE total, ~754M circulating. 25% investors (vested), 25% founders (vested), 50% community. Low voter turnout reported. Insider allocation of ~50% suggests moderate concentration. However, Dual Approval Governance materially addresses concentration risk by giving Vault owners (non-SWISE) full veto power - refusal to upgrade completely blocks any upgrades. This is a significant structural mitigation unique among DeFi protocols. | 9 | 9 | Non-Improvable | [P3] CoinMarketCap: distribution |
| O-G-03 | Are mechanisms in place to limit major changes? | SafeSnap: 48h effective delay (24h timeout + 24h cooldown). 200K SWISE bond. 4-of-7 multisig can reject transactions (4 required to reject). 48h exceeds 24h minimum. Every Vault is free to adopt changes individually with no time limit. | 9 | 9 | Non-Improvable (optimal) | [P1] SafeSnap parameters, [P0] SafeSnap module |
...
| O-TL-02 | Is the protocol dependent on a single developer? | No. Large, diverse contributor ecosystem. Multiple independent development teams. 100+ node operators across 40 Vaults. Vault model enables permissionless validator participation. Protocol can function with multiple teams contributing independently. 11 oracle operators provide service decentralization. | 9 | 9 | Non-Improvable (optimal) | [P1] [GitHub v3-core](https://github.com/stakewise/v3-core) |
| O-TL-03 | What legal entity and jurisdiction operate the protocol? | Decentralized DAO structure for the open-source protocol. Creative Tech FZCO incorporated in the UAE operates StakeWise front-end to enable access to smart contracts. | 9 | 9 | Non-Improvable (optimal) | [P1] [StakeWise Terms of Service](https://app.stakewise.io/terms-and-conditions): UAE entity confirmed |
...
| O-TL-05 | Is there an on-call and incident response process? | Yes, demonstrated in practice. October 2025 Balancer hack: emergency multisig powers invoked within hours, 73.5% of stolen osETH was recovered from the hacker, post-mortem published. Governance multisig on stand by, explicitly mandated to step in to veto malicious transactions submitted to the SafeSnap module. Oracle set operates under strict SLAs. However, formal 24/7 on-call documentation with specific SLAs not publicly available. | 9 | 9 | Non-Improvable (optimal) | [P1] [StakeWise Governance Docs](https://docs.stakewise.io/docs/governance/dao-treasury#current-committee-members-multisig), [P1] [Oracle set SLAs mentioned](https://forum.stakewise.io/t/stakewise-v3-oracle-network/1321) |
...
- Raw scores: 9+9+9+9+9+9+9 = 63/63
- Adjusted: (63/63) x 67.5 = **67.5/67.5 (100%)**
...
| O-DT-04 | Are roles, permissions and timelocks documented? | SafeSnap parameters documented. Multisig signers verifiable via Safe API (4-of-7, 7 owners). OsToken owner documented. Vault permissions configurable at the individual Vault level. SWIPs serve as change log to the protocol parameters. | 9 | 9 | Non-Improvable (optimal) | [P0] Safe API: signer addresses verifiable, [P2] SWIPs |
...
| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| O-FR-01 | Is there a backstop reserve or safety module for user losses? | No dedicated safety module or backstop. DAO treasury (~$2.3M) has no binding activation rules for user loss coverage. Overcollateralization (10%) is per-position, not a collective backstop. No insurance fund. Note: DAO multisig did recover ~$19M osETH within 90 min during Balancer hack (Nov 2025), demonstrating ad-hoc crisis response capability. | 3 | 9 | **Improvable** | [P0] Treasury: ~$2.3M, [P1] No safety module, [P1] Balancer recovery |
| O-FR-02 | How large are backstop reserves relative to TVL? | DAO treasury ~$2.3M (osETH $1M + SWISE $1.3M). osETH total supply ~$400-450M. Ratio: ~0.5%. Below 1% threshold. Large share in SWISE (illiquid). DeFiLlama treasury API returns no data. | 6 | 9 | **Improvable** | [P0] [Treasury](https://etherscan.io/address/0x144a98cb1CdBb23610501fE6108858D9B7D24934), [P3] DeFiLlama: no data |
| O-FR-03 | What is the estimated operational runway? | Strong. Protocol generating revenue from 5% treasury fee on ~$450M TVL (~2.5-3% APR staking rewards = ~$0.7M annual revenue to treasury). Total 2025 spending at $1.1M according to forum post. $1M liquid treasury balance reported March 2026. Runway comfortably >24 months. Trending toward profitability. | 9 | 9 | Non-Improvable (optimal) | [P3] [DefiLlama revenue](https://defillama.com/protocol/stakewise): ~$675K annual, [P0] 5% fee on staking rewards, [P3] [Operating costs breakdown for 2025](https://forum.stakewise.io/t/swip-31-issue-development-grant-for-stakewise-labs/1881) |
| O-FR-04 | How have TVL, revenue and buffers behaved in past stress? | V3 TVL grew from launch to ~$881M. Balancer hack (Nov 2025) impacted LP pools but not protocol TVL directly. No major stress-driven TVL collapse. TVL correlates with ETH price. No buffer depletion. Protocol tested in a severe crypto-wide downturn multiple times since launching in 2021. | 9 | 9 | Non-Improvable (optimal) | [P3] [DefiLlama TVL](https://defillama.com/protocol/stakewise) |
| O-FR-05 | Can the protocol remain safe if team disappears? | Core contracts non-upgradeable. Oracle network requires ongoing operation by 11 operators. If team disappears: main repo maintainer is lost but osETH redeemable on-chain, oracles continue reporting rewards, multisig continues on standby. Vault withdrawals permissionless. DEX exits available. | 9 | 9 | Non-Improvable (optimal) | [P0] Non-upgradeable contracts |

**Financial Resilience Subtotal:**
- Raw scores: 3+6+9+9+9 = 36/45
- Adjusted: (36/45) x 67.5 = **54.0/67.5 (80.0%)**

**Operations Total: 67.5 + 67.5 + 67.5 + 54.0 = 256.5/270 (95.0%)**

## Final Score Calculation

| Category | Subcategory | Raw Score | Max Raw | Adj Points | Max Points | Percentage |
|----------|-------------|-----------|---------|------------|------------|------------|
| **Security** | Smart Contract Security (11 Q) | 99 | 99 | 180 | 180 | 100% |
| | Key Management (7 of 8 Q scored) | 63 | 63 | 180 | 180 | 100% |
| | **Security Subtotal** | | | **360** | **360** | **100%** |
| **Strategy** | Protocol Mechanics (7 of 9 Q scored) | 63 | 63 | 45.0 | 45 | 100.0% |
| | Collateral (4 Q) | 36 | 36 | 45.0 | 45 | 100.0% |
| | Infra Counterparty (8 Q) | 72 | 72 | 45 | 45 | 100% |
| | Protocol Counterparty (4 of 5 Q scored) | 36 | 36 | 45.0 | 45 | 100.0% |
| | Liquidity (9 Q) | 81 | 81 | 45 | 45 | 100% |
| | Market (5 Q) | 39 | 45 | 39.0 | 45 | 86.7% |
| | **Strategy Subtotal** | | | **264** | **270** | **97.8%** |
| **Operations** | Governance (3 of 4 Q scored) | 27 | 27 | 67.5 | 67.5 | 100% |
| | Team & Legal (7 of 9 Q scored) | 63 | 63 | 67.5 | 67.5 | 100% |
| | Documentation (6 Q) | 54 | 54 | 67.5 | 67.5 | 100% |
| | Financial Resilience (5 Q) | 36 | 45 | 54 | 67.5 | 80.0% |
| | **Operations Subtotal** | | | **256.5** | **270** | **95.0%** |
| **TOTAL** | | | | **880.5** | **900** | **97.8%** |
