...
**NOTE: S-SC-10 is scored 3 under the v0.1-beta framework verification principle (monitoring exists but cannot automatically trigger protective actions -- manual human intervention required; Public Allocator flows not covered by automated halt). This is consistent with the Smokehouse USDC rating under the same framework. The prior rating scored S-SC-10 as 9.**
...
| S-SC-08 | Were there reductions to the timelock delay that weaken governance protections? | No reductions. 7-day timelock maintained on Steakhouse vaults (above the 3-day protocol minimum). No changes to timelock duration in last 12 months. | 9 | 9 | Non-Improvable (optimal) | [P1] [Vault Controls](https://www.steakhouse.financial/docs/risk-management/monitoring/vault-setup-and-controls) |
...
| S-SC-11 | Are automatic safety controls (pause, circuit breakers) triggered by monitoring alerts? | No automatic on-chain circuit breakers by design. Morpho Blue prioritizes immutability over admin controls. Guardian can manually veto pending timelocked actions but cannot pause the protocol. Market-level isolation provides containment. Circuit breakers do not cover automated allocation mechanisms (Public Allocator). | 3 | 3 | Non-Improvable | [P1] [Security Framework](https://morpho.org/blog/morpho-blue-security-framework-building-the-most-secure-lending-protocol/) |
...
| S-KM-06 | Are user funds fully segregated from treasury and operational wallets? | Yes. Clear on-chain segregation. Morpho is a nonprofit with no profit extraction from protocol operations. Vault funds are held in Morpho Blue markets, separate from any treasury or operational wallets. On-chain fee is 0% (confirmed via Morpho API, March 2026). | 9 | 9 | Non-Improvable (optimal) | [P0] [Morpho API](https://blue-api.morpho.org/graphql), [P1] [Legal Notice](https://morpho.org/legal-notice/), [P0] [Etherscan](https://etherscan.io/address/0xbeef01735c132ada46aa9aa4c54623caa92a64cb) |
| S-KM-07 | What are the whitelisted protocols the vault strategy can interact with? | On-chain enforced whitelist of Morpho Blue markets only. Current markets: cbBTC/USDC, WBTC/USDC, wstETH/USDC. New market additions require Curator approval + 7-day timelock + Guardian veto. Supply caps per market enforced on-chain. | 9 | 9 | Non-Improvable (optimal) | [P3] [Morpho App](https://app.morpho.org/ethereum/vault/0xBEEF01735c132Ada46AA9aA4c54623cAA92A64CB/steakhouse-usdc), [P1] [Morpho Roles](https://docs.morpho.org/curate/concepts/roles/) |
...
### STRATEGY (30% Weight) -- Score: 239.2/270 (88.6%)

#### Protocol Mechanics (5% weight, 45 max points)

| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| ST-PM-01 | What are the sources of yield and is it aligned with the mandate? | All yield from overcollateralized USDC lending against blue-chip crypto collateral (cbBTC, WBTC, wstETH). 100% mandate-aligned. Yield sources explicitly documented on Morpho App with market-level breakdown. | 9 | 9 | Non-Improvable (optimal) | [P3] [Morpho App](https://app.morpho.org/ethereum/vault/0xBEEF01735c132Ada46AA9aA4c54623cAA92A64CB/steakhouse-usdc) |
...
| ST-PM-04 | Are there automated mechanics to prevent Liquidation Events? | N/A -- Vault is a lender, not a borrower. Liquidation risk applies to borrowers. Vault benefits from liquidation incentive fees (5% LIF at 86% LLTV). Pre-liquidation mechanism available for borrowers who opt in. | N/A | N/A | N/A | [P1] [Liquidation](https://docs.morpho.org/learn/concepts/liquidation/) |
| ST-PM-05 | Has this strategy or a close variant ever experienced negative yield? | No permanent-loss events for Steakhouse USDC vault or comparable Steakhouse vaults since January 2024 launch. During November 2025 Stream/Elixir contagion, Steakhouse vaults had ZERO bad debt (confirmed by Steakhouse). MEV Capital vault was the one affected (3.6% bad debt). Steakhouse experienced only temporary illiquidity. | 9 | 9 | Non-Improvable (optimal) | [P1] [Steakhouse Docs](https://www.steakhouse.financial/docs/products/steakhouse-financial-products/morpho-vaults), [P4] [CryptoRank](https://cryptorank.io/news/feed/275ce-risk-curators-took-off-in-2025-but-led-to-recent-defi-lending-vault-troubles) |
...
| ST-PM-07 | Is current yield sustainable relative to underlying economics? | Yes. Yield is entirely organic from USDC lending demand driven by BTC/ETH leveraged borrowers. No emissions or token incentives required. On-chain fee for this vault is 0% (per Morpho API, March 2026). Coinbase partnership drives sustainable demand (Coinbase crypto-backed loans). Protocol generates $300K/week in curator fees across ecosystem. | 9 | 9 | Non-Improvable (optimal) | [P1] [Morpho 2025 Review](https://morpho.org/blog/the-morpho-effect-2025/), [P0] [Morpho API](https://blue-api.morpho.org/graphql) |
| ST-PM-08 | Can the position be fully unwound under conservative liquidity assumptions without slippage turning yield negative? | Yes. USDC is the most liquid stablecoin. During November 2025 stress, approximately 80% of Steakhouse vault withdrawals processed within 3 days. Slippage limited to rate model dynamics (4x rate at 100% utilization incentivizes rapid repayment). No slippage on the USDC asset itself. | 9 | 9 | Non-Improvable (optimal) | [P4] [CryptoRank](https://cryptorank.io/news/feed/275ce-risk-curators-took-off-in-2025-but-led-to-recent-defi-lending-vault-troubles) |
...
| ST-PM-10 | Can automated or externally-invocable mechanisms increase vault exposure without real-time human approval? | Yes. The vault uses automated allocation via the Public Allocator and Curator-appointed allocators. Supply caps per market limit maximum per-market exposure. However, no automatic kill-switch exists for abnormal conditions (oracle deviation, collateral depeg, utilization spikes). The March 2026 Resolv exploit demonstrated that the Public Allocator on Morpho vaults can auto-supply funds to broken markets for hours because anyone can invoke the function and no circuit breaker halts it. While Steakhouse markets were not directly affected, the same Public Allocator infrastructure applies. Halting requires manual curator intervention (setting caps to zero). | 3 | 9 | **Improvable** | [P1] [Morpho Public Allocator](https://docs.morpho.org/curate/concepts/public-allocator/), [P1] [Morpho Roles](https://docs.morpho.org/curate/concepts/roles/) |
...
| ST-C-04 | What share of TVL relies on bridged or wrapped assets? | HIGH RISK: Approximately 98% of vault TVL is in wrapped assets (primarily cbBTC and WBTC markets). Per the v0.1-beta framework definition, "bridged or wrapped" includes third-party bridge tokens and wrapped representations like WBTC/cbBTC backed by custody. Protocol receipt tokens like wstETH are excluded. cbBTC (Coinbase custody, NY DFS regulated) and WBTC (BitGo/BiT Global, Justin Sun controversy) remain wrapped BTC representations. Exceeds 60% threshold significantly. | 1 | 9 | **Improvable** | [P3] [Morpho App](https://app.morpho.org/ethereum/vault/0xBEEF01735c132Ada46AA9aA4c54623cAA92A64CB/steakhouse-usdc), [P4] [The Block - WBTC](https://www.theblock.co/post/357379/justin-sun-connected-bit-global-drops-wbtc-related-legal-spat-with-coinbase) |
...
| ST-IC-02 | How redundant are oracle and bridge setups, and can oracle values be corrected under abnormal conditions? | SINGLE ORACLE RISK with immutable pricing. Each Morpho Blue market has one immutable oracle -- no fallback mechanism possible post-deployment and oracle values cannot be updated, overridden, or corrected after deployment. Per the v0.1-beta mandatory hardcoded/immutable oracle check: the oracles price battle-tested, deep-liquidity assets (BTC, ETH against USDC) where material depeg is historically unprecedented. Chainlink is Tier-0, but zero redundancy and zero ability to correct under abnormal conditions by design. Caps at Mid (3) per framework. | 3 | 3 | Non-Improvable | [P1] [Oracle Docs](https://docs.morpho.org/learn/concepts/oracle/) |
...
| ST-IC-04 | How did these infra components behave in past outages or chain incidents? | Ethereum: No significant halts affecting operations. Morpho: April 2025 frontend vulnerability resolved in 4 minutes. November 2025 stress: protocol operated correctly (market isolation worked, liquidations processed, no system failures). No blocking of on-chain withdrawals. | 9 | 9 | Non-Improvable (optimal) | [P1] [Incident Report](https://morpho.org/blog/morpho-app-incident-april-10-2025/) |
...
| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| ST-PC-01 | Into which external protocols, CEXs or RWA platforms is collateral deployed? | All allocation to Morpho Blue markets only (Tier-0 protocol, 25+ audits, formally verified). No CEX exposure. No RWA platforms. Markets clearly listed with on-chain caps. | 9 | 9 | Non-Improvable (optimal) | [P3] [Morpho App](https://app.morpho.org/ethereum/vault/0xBEEF01735c132Ada46AA9aA4c54623cAA92A64CB/steakhouse-usdc) |
| ST-PC-02 | Are revenue-share or incentive arrangements documented and do they create conflicts? | Yes. On-chain fee is 0% (confirmed via Morpho GraphQL API, March 2026). Fee changes require timelock. No hidden rebates or undisclosed arrangements. Coinbase partnership is public (Coinbase crypto-backed loans). | 9 | 9 | Non-Improvable (optimal) | [P0] [Morpho API](https://blue-api.morpho.org/graphql), [P2] [Forum](https://forum.morpho.org/t/key-information-on-steakhouse-branded-vaults/1343) |
| ST-PC-03 | Are there hard limits and rebalancing rules per counterparty? | Yes. On-chain supply caps per market enforced by MetaMorpho contract. Allocator rebalances within caps. New market additions require Curator + timelock + Guardian. Cap changes subject to timelocks for increases. | 9 | 9 | Non-Improvable (optimal) | [P1] [Morpho Roles](https://docs.morpho.org/curate/concepts/roles/) |
...
| ST-M-03 | Are interest rate/incentive models robust during TVL shifts or volatility spikes? | Yes. Immutable Adaptive Curve IRM per market. Rates auto-increase with utilization (4x multiplier at 100% utilization). No governance manipulation possible. Rate curves become steep as utilization approaches critical levels. No history of prolonged >=95% utilization beyond brief stress episodes. | 9 | 9 | Non-Improvable (optimal) | [P1] [Risk Docs](https://docs.morpho.org/learn/resources/risks) |
| ST-M-04 | Have historic scenarios with high market volatility been explicitly modeled? | Yes. 86% LLTV provides 14% buffer before liquidation. Liquidation mechanism formally verified by Certora (mathematical proofs of correctness). Pre-liquidation mechanism available for additional safety. Certora proved reentrancy safety, market independence, arithmetic integrity, and withdrawal safeguards. | 9 | 9 | Non-Improvable (optimal) | [P1] [Formal Verification](https://morpho.org/blog/formally-verifying-morpho-blue-with-certorav/) |
...
| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| O-TL-01 | Are core team and operating entities publicly identified and credible? | Yes. Steakhouse: adcv (co-founder, former M&A/investment banking), Sebastien Derivaux (co-founder, former MakerDAO Head of RWA). Morpho: Paul Frambot (CEO), Merlin Egalite, Mathis Gontier Delaunay. All publicly identified with verifiable professional backgrounds and track records. | 9 | 9 | Non-Improvable (optimal) | [P1] [Steakhouse](https://www.steakhouse.financial/), [P3] [Crunchbase](https://www.crunchbase.com/person/s%C3%A9bastien-derivaux-af07), [P1] [Morpho](https://morpho.org/) |
...
| O-TL-03 | What legal entity and jurisdiction operate the protocol and/or assets? | Morpho: ADDMO (French nonprofit, W751263773, 24 rue de Clichy, 75009 Paris) + Morpho Labs SAS (France, RCS 902 498 492). Steakhouse: Carniciera Tropical Inc. (Panama) + Steakhouse Financial Ltd (Cayman Islands). All entities clearly identified with registration numbers where available. | 9 | 9 | Non-Improvable (optimal) | [P1] [Legal Notice](https://morpho.org/legal-notice/), [P2] [Forum](https://forum.morpho.org/t/key-information-on-steakhouse-branded-vaults/1343) |
...
| O-TL-09 | Has the strategy manager previously been involved in products with collateral loss? | No. Zero bad debt across all Steakhouse-curated vaults since inception (January 2024). November 2025 stress: Steakhouse vaults specifically confirmed zero bad debt while other curators (MEV Capital) experienced losses. Clean 2+ year track record. | 9 | 9 | Non-Improvable (optimal) | [P1] [Steakhouse Docs](https://www.steakhouse.financial/docs/products/steakhouse-financial-products/morpho-vaults), [P4] [CryptoRank](https://cryptorank.io/news/feed/275ce-risk-curators-took-off-in-2025-but-led-to-recent-defi-lending-vault-troubles) |
...
**Improvement Opportunities:**
- **O-TL-05** (+5.0 weighted points): Publish documented 24/7 on-call procedures with defined escalation and response SLAs, including procedures to halt automated allocation mechanisms
...
| Code | Question | Answer Summary | Current | Potential | Classification | Evidence |
|------|----------|----------------|---------|-----------|----------------|----------|
| O-DT-01 | Is there up-to-date documentation explaining architecture, strategy mechanics, and risks? | Yes. Morpho Docs comprehensive and regularly updated (V2 documentation added throughout 2025). Steakhouse publishes vault controls, risk management docs, and regular Substack updates (Kitchen newsletter). Strategy mechanics clearly explained. | 9 | 9 | Non-Improvable (optimal) | [P1] [Morpho Docs](https://docs.morpho.org/), [P1] [Steakhouse Docs](https://www.steakhouse.financial/docs/risk-management/monitoring/vault-setup-and-controls) |
...
**Improvement Opportunities:**
- **O-FR-01** (+9.0 weighted points): Create dedicated safety module funded by portion of performance fees with clear, binding activation rules
...
| | **Strategy Subtotal** | | | **239.2** | **270** | **88.6%** |
